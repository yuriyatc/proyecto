...
class Categoria extends Conectar{
    public function get_categoria(){
        ...
        parent::set_names();
        $sql="SELECT * FROM clients WHERE activo = 'si' ";
    ...
    public function get_categoria_x_id($id){
        $conectar= parent::conexion();
        parent::set_names();
        $sql="SELECT * FROM clients WHERE activo = 'si' AND id = ?";
        $sql=$conectar->prepare($sql);
        $sql->bindValue(1, $id);
        $sql->execute();
        return $resultado=$sql->fetchAll(PDO::FETCH_ASSOC);
    ...
    public function insert_categoria($nombre,$direccion,$poblacion,$estado,$telefono,$descripcion,$fecha_inicio){
        $conectar= parent::conexion();
        parent::set_names();
        $sql="INSERT INTO clients(id,nombre,direccion,poblacion,estado,telefono,descripcion,fecha_inicio,activo) VALUES (null,?,?,?,?,?,?,?,'si');";
        $sql=$conectar->prepare($sql);
